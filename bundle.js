...
const App = {
        ...
        content.innerHTML = `
            ...
            <div class="card budget-overview-card" style="margin-bottom: 2rem; cursor: default; padding: 2rem;">
                ...
                return `
                    ...
                    ${totalActual > 0 && totalExpected > 0 ? (() => {
                        const exactlyOnBudget = totalActual === totalExpected;
                        const overBgt = totalActual > totalExpected;
                        const savedAmt = totalExpected - totalActual;
                        const savedPct = Math.round((savedAmt / totalExpected) * 100);
                        const overspentAmt = totalActual - totalExpected;
                        if (exactlyOnBudget) {
                            return `<div style="padding:1rem 1.5rem;border-radius:12px;font-size:0.9rem;text-align:center;margin-top:1.5rem;background:rgba(59,130,246,0.1);color:#93c5fd;border:1px solid rgba(59,130,246,0.2);box-shadow:0 0 20px rgba(59,130,246,0.1);font-weight:500;">ℹ️ Budget fully used — ₹0 remaining</div>`;
                        } else if (overBgt) {
                            return `<div style="padding:1rem 1.5rem;border-radius:12px;font-size:0.9rem;text-align:center;margin-top:1.5rem;background:rgba(239,68,68,0.1);color:#fca5a5;border:1px solid rgba(239,68,68,0.2);box-shadow:0 0 20px rgba(239,68,68,0.1);font-weight:500;">⚠️ Over budget by ${UIComponents.formatCurrency(overspentAmt)} — review your expenses</div>`;
                        } else {
                            return `<div style="padding:1rem 1.5rem;border-radius:12px;font-size:0.9rem;text-align:center;margin-top:1.5rem;background:rgba(16,185,129,0.1);color:#6ee7b7;border:1px solid rgba(16,185,129,0.2);box-shadow:0 0 20px rgba(16,185,129,0.1);font-weight:500;">✅ You saved ${UIComponents.formatCurrency(savedAmt)} — ${savedPct}% of budget remaining</div>`;
                        }
                    })() : ''}
                    `;
            })()}
    ...
    createDayCard(day) {
        ...
        return `
            <div class="card" style="margin-bottom: 1rem; cursor: default; border-left: 4px solid ${accentColor}; position:relative; display: flex; flex-direction: column;">
